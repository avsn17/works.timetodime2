...
class PomodoroTimer:
    ...
    def draw_ui(self):
        ...
        for row_idx in range(rows - 1):
            ...
            if row_idx == 1:
                for i, char in enumerate(header[:len(line)]): line[i] = char
            elif row_idx == 3:
                for i, char in enumerate(timer_display): line[2 + i] = char
            elif row_idx == 5:
                bar_plain = f"[{'█' * filled}{'░' * (progress_bar_width - filled)}] {distance_covered:.0f}m"
                for i, char in enumerate(bar_plain): line[2 + i] = char
            elif row_idx == 7:
                for i, char in enumerate(status): line[2 + i] = char
    ...
    def timer_complete(self):
        ...
        print(f"\n\n{COLORS['solar']}🎉 Goal Reached!{COLORS['reset']}")
        print(f"✨ Wisdom: {random.choice(QUOTES['kirby'])}")
        # Signal web for music
    ...
    def show_stats(self):
        ...
        if not self.stats:
            ...
            for rank, (name, data) in enumerate(sorted(self.stats.items(), key=lambda x: x[1]['total_distance'], reverse=True), 1):
    ...
    def open_settings(self):
        ...
        print('★' * 15)
        print(f'[1] Hydration Interval (Current: {self.remind_interval}m)')
    ...
    def run(self):
        self.clear_screen()
        print(f"{COLORS['solar']}🌟 Cosmic Pomodoro 🌟{COLORS['reset']}\n")
        
        while True:
            try:
                distance = int(input(f"\n{COLORS['green']}Enter distance goal (meters): {COLORS['reset']}"))
            ...
            while self.running or self.paused:
                ...
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key == ' ':
                        ...
                        state = 'toggle' if self.music_playing else 'pause'
        ...
        self.run() # Restart loop for 'N' or completion
